<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            ...
            <div>
                <h2 class="text-3xl font-extrabold leading-tight text-slate-900 dark:text-slate-100 mt-1">{{ __('Edit Kategori Utama') }}</h2>
                <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Perbarui rincian kategori pembayaran utama.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 sm:py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl p-6 sm:p-10 transition-colors">
                <form method="POST" action="{{ route('admin.payment-categories.update', $paymentCategory) }}" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <x-input-label for="name" :value="__('Nama Kategori Utama')" />
                        <x-text-input id="name" class="block mt-1.5 w-full" type="text" name="name" :value="old('name', $paymentCategory->name)" required autofocus />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="sort_order" :value="__('Urutan Tampil (Sort Order)')" />
                        <x-text-input id="sort_order" class="block mt-1.5 w-full" type="number" name="sort_order" :value="old('sort_order', $paymentCategory->sort_order)" required />
                        <x-input-error :messages="$errors->get('sort_order')" class="mt-2" />
                    </div>

                    <div class="pt-2">
                        <label for="is_active" class="inline-flex items-center cursor-pointer">
                            <input id="is_active" type="checkbox" class="w-5 h-5 rounded border-slate-300 text-blue-600 shadow-sm focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:checked:bg-blue-500 dark:focus:ring-blue-500 transition-colors cursor-pointer" name="is_active" value="1" {{ old('is_active', $paymentCategory->is_active) ? 'checked' : '' }}>
