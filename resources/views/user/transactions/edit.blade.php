<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            <p class="text-sm font-bold uppercase tracking-[0.24em] text-blue-600 dark:text-blue-400">Edit Transaksi</p>
            <div>
                <h2 class="text-3xl font-extrabold leading-tight text-slate-900 dark:text-slate-100 mt-1">Perbarui Catatan</h2>
                <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Sesuaikan rincian transaksi Anda yang sudah ada.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 sm:py-8" x-data="{ 
        ...
        totalAmount: '',
        payments: {{ json_encode(old('payments', $transaction->payments->map(fn($p) => ['payment_method_id' => $p->payment_method_id, 'amount' => $p->amount]))) }},
        ...
        init() {
            this.totalAmount = this.formatDisplay({{ old('amount', $transaction->amount) }});
            this.payments = this.payments.map(p => ({
                ...p,
                amount: this.formatDisplay(p.amount)
            }));
        },
                ...
                <form method="POST" action="{{ route('transactions.update', $transaction) }}" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="transaction_date" type="date" name="transaction_date" value="{{ old('transaction_date', $transaction->transaction_date->format('Y-m-d')) }}" class="mt-1.5 w-full" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="description" type="text" name="description" value="{{ old('description', $transaction->description) }}" class="mt-1.5 w-full" placeholder="Contoh: Gaji bulanan / Makan siang" required autocapitalize="sentences" spellcheck="true" />
                    ...
                    <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end border-t border-slate-100 dark:border-slate-800 pt-6 mt-6">
                        ...
                        <button type="submit" 
                                ...
                                class="inline-flex w-full sm:w-auto items-center justify-center rounded-xl bg-blue-600 px-6 py-3 text-sm font-black text-white shadow-lg shadow-blue-900/20 transition hover:bg-blue-700 hover:-translate-y-0.5">
                            Update Transaksi
                        </button>
                    </div>
                </form>
